...
class UE:
    ...
    def __init__(self, host, name, ip, port, link_e, num, gains=0.0, F_BS=0.0, F_UE=0.0, N1=0.0, b=0.0, Power=0,
                 flag=False):
        self.host = host
        self.name = name
        self.ip = ip
...
def topology():
    ...
    while round < Total_r:
        ...
        if (queue[num].N1 * 0.00004) < queue[num].Power:
            ...
            print('the %d-th device has been selected, power from %f to %f' % (
                queue[num].num, queue[num].Power, queue[num].Power - queue[num].N1 * 0.00004))
    ...
    plt.figure()
    round = [k for k in range(0, Total_r)]
